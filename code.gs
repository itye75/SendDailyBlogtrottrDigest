...
function sendDailyBlogtrottrDigest() {
  ...
  for (var i = 0; i < threads.length; i++) {
    ...
    for (var j = 0; j < messages.length; j++) {
      ...
      // Filter for likely article links, but only take the FIRST valid one
      var filtered = links
      ...
      // Only take the first valid link
      var firstUrl = filtered[0];

      // Use the message id to ensure each email message is processed once
      if (seen[m.getId()]) continue;
      seen[m.getId()] = true;

      var date = m.getDate();
      var source = tryExtractSourceFromEmail_(subject, html, firstUrl);

      items.push({
        emailDate: date,
        subject: subject,
        url: firstUrl,      // <-- single URL string now
        source: source,
        digest: '',
        title: ''
      });
  ...
  var subjectLine = 'Blogtrottr Digest · ' + todayStr;
  var htmlOut = renderHtmlTable_(items);
  var textOut = renderTextTable_(items);

  logItems(items);

  GmailApp.sendEmail(RECIPIENT, subjectLine, textOut, { htmlBody: htmlOut });
}
...
function buildPageSummaries_(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    // Attempt to extract the title from the fetched HTML.
...
function callGeminiSummarize_(apiKey, url, source, text) {
  try {
    ...
    if (resp.getResponseCode() >= 200 && resp.getResponseCode() < 300) {
      ...
      return out; // Keep newlines for bullets
    }
...
function renderHtmlTable_(items) {
  ...
  var rows = items.map(function(it) {
    ...
    // Single URL cell
    var safeUrl = escapeHtml_(String(it.url || ''));
    var urlCell = safeUrl
    ...
    return '<tr><td style="white-space:nowrap; vertical-align:top; padding:6px 8px">' + d + '</td>' +
           '<td style="vertical-align:top; padding:6px 8px">' + safeSource + '</td>' +
           '<td style="vertical-align:top; padding:6px 8px">' + safeTitle + '</td>' +
           '<td style="vertical-align:top; padding:6px 8px">' + urlCell + '</td>' +
           '<td style="vertical-align:top; padding:6px 8px">' + digestHtml + '</td></tr>';
  }).join('\n');
...
function renderTextTable_(items) {
  ...
  var lines = items.map(function(it) {
    ...
    // Single URL string
    var urlLine = String(it.url || '').trim();

    return 'Email date: ' + d +
           '\nSource name: ' + it.source +
           '\nPost title: ' + it.title +
           '\nLink to post: ' + urlLine +
